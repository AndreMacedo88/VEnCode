...
from utils.util import file_names_to_list
...
def write_dict_to_csv(file_name, dict_data, folder=None, path="normal", deprecated=True):
    ...
    if deprecated:
        path_working = utils.directory_handlers.path_handler(path)
        try:
            ...
            print(message)
            raise
        utils.directory_handlers.check_if_and_makedir(path_working + folder)
    else:
        new_file = file_name
...
def write_dict_2_to_csv(file_name, dict_data, folder, path="normal"):
    path_working = utils.directory_handlers.get_path(path)
    try:
        new_file = path_working + folder + file_name
    except Exception as ex:
        template = "An exception of type {0} occurred. Arguments:\n{1!r}"
