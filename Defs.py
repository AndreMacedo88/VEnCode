...
def write_dict_2_to_csv(file_name, dict_data, folder):
    current_path = os.getcwd()
    new_file = current_path + folder + file_name
    keys = sorted(dict_data.keys())
    rows = list(zip_longest(*dict_data.values()))
    with open(new_file, 'w') as csv_file:
        writer = csv.writer(csv_file, delimiter=";", lineterminator='\n')
        writer.writerow(keys)
...
def fantom_percentage_of_zeros_calculator(data_frame, celltype):
    ...
    for index, series in data_frame.drop(celltype, axis=1).iterrows():
        ...
        for p in series.values.tolist():
            if p == 0:
                ...
                pass
        percent = percentage(x, len(data_frame_2.columns))
        percent_list.append(percent)
    column_label = "percentage_of_zeros"
    data_frame_2[column_label] = percent_list
    return data_frame_2, column_label


def fantom_percentile_calculator(data_frame, celltype, start_percent, define_percentile=False):
    """
...
def fantom_filter_1(data, codes, expression):
    ...
    if isinstance(codes, list):
        ...
        filter_1 = filter_1[filter_1[codes] >= expression]
    return filter_1


def fantom_filter_2(data_frame, column):
    """ Returns a df containing only the rows that present a value = 0 for the column """
    filter_2 = data_frame[data_frame[column] == 0]
    return filter_2


def fantom_filters(data, codes, expression, amount, threshold=None):
    """ Returns a df after applying fantom_filter_1 function or both fantom_filter_1 and _2 functions """
    filter_1 = fantom_filter_1(data, codes, expression)
    print("After filter 1: ", filter_1.shape[0])
    logging.info("After filter 1: %s promoters", filter_1.shape[0])
    if amount == 1:
        return filter_1
    elif amount == 2:
        with_percentile, column_name = fantom_percentile_calculator(filter_1, codes, threshold)
            ...
            for i in range(reps):
                ...
                for n in range(samples_to_take):
                    ...
                    if include_problems:
                        ...
                        c = quick_assess_vencode_one_zero_upd(c, sample_dropped)
...
def quick_assess_vencode_one_zero_upd(counter, sample):
    """
...
def dataframe_regex_searcher(string, database):
    ...
    regular = ".*" + string.replace(" ", ".*").replace("+", "%2b").replace(":", "%3a").replace("(", "%28").replace(")", "%29").replace("/", "%2f") + ".*"
...
def number_of_combination_from_nodes(node_dict, n, r):
    ...
    if r > 2:
        for i in node_dict.keys():
            nodes = node_dict[i]
            combinations += (comb((n - 2), (r - 2), exact=False) * nodes) - comb(nodes, 2)  # see triangular numbers
...
def sorted_ven_robustness_test(file, file_type, celltype, combinations_number, samples_to_take, reps, threshold=90,
    ...
    if init_data is None:
        ...
        to_keep = sample_category_selector("sample types - FANTOM5.csv", sample_types)
    ...
    if isinstance(expression, list):
        ...
        for item in expression:
            ...
            filter_2 = fantom_filters(data, codes, item, 2, threshold)
            k_ven_percent_2, problems = fantom_sampling(codes, celltype, filter_2, combinations_number_list,
        ...
        filter_2 = fantom_filters(data, codes, expression, 2, threshold)
        k_ven_percent, problems = fantom_sampling(codes, celltype, filter_2, combinations_number_list,
                                                  samples_to_take,
...
__copyright__ = "GPL"
__credits__ = ["Andre Macedo", "Alisson Gontijo", "Fabiana Heredia", "Andreia Casimiro"]
__license__ = "GPL"
__version__ = "1.0.1"
