...
def enhancers_improved_vencodes(names_db, file, celltype, p=None, k=4, n=None, write_file=False):
    ...
    codes = Utils.fantom_code_selector("Enhancers", names_db, celltype)
    if len(codes) == 1:
        filter_1 = Utils.df_filter_by_expression(raw_data, codes, 1)
        print(filter_1.shape[0])
        threshold = 80
        with_percentage_of_zeros, column_name = Utils.df_percentile_calculator(filter_1, codes, threshold)
        ...
        Utils.reform_vencode_n_combinations_of_k(threshold, sorted_1.drop(column_name, 1),
        ...
        for i in range(0, len(codes)):
            ...
            print(cropped.shape)
            with_sum = Utils.other_lines_sum_generator(cropped, code)
            p_probes_sorted_df = with_sum.nsmallest(p, "sum")
            Utils.reform_vencode_n_combinations_of_k(p_probes_sorted_df.drop("sum", 1), code, "Enhancers", k, n,
                                                     write_file)
    return
...
def promoters_improved_vencodes(file, celltype, p=None, k=4, n=None, write_file=False):
    ...
    codes = Utils.fantom_code_selector("Promoters", data, celltype)
    print(codes)
    if len(codes) == 1:
        code = codes[0]
        others_df = None
        filter_1 = Utils.df_filter_by_expression(data, codes, 1)
        threshold = 100
        while threshold > 0:
            with_percentage_of_zeros, column_name = Utils.df_percentile_calculator(filter_1, codes, threshold)
            filter_2 = Utils.df_filter_by_column_value(with_percentage_of_zeros, column_name)
...
def multiple_ven_robustness_test(file, file_type, celltype, combinations_number, samples_to_take, reps, zeros):
    ...
    data = raw_data.drop(raw_data.index[[0, 1]])
    codes = Utils.fantom_code_selector(file_type, data, celltype)
    print(codes)
    code = codes[0]
    codes_2 = np.array(codes).tolist()
    codes_2.remove(code)
...
def ven_robustness_test(file, file_type, celltype, combinations_number, samples_to_take, reps, expression=2):
    ...
    codes = Utils.fantom_code_selector(file_type, data, celltype)
    print(codes)
    filter_1 = Utils.df_filter_by_expression_and_percentile(data, codes, expression, 1)
    k_ven_percent = Utils.vencode_percent_sampling(codes, celltype, filter_1, combinations_number, samples_to_take,
                                                   reps)
    folder = "/Percentage/"
...
def sorted_ven_robustness_test(file, file_type, celltype, combinations_number, samples_to_take, reps, threshold=90,
    ...
    if init_data is None:
        ...
        data = init_data
    codes = Utils.fantom_code_selector(file_type, data, celltype, not_include=not_include)
    ...
    if isinstance(expression, list):
        ...
        for item in expression:
            ...
            filter_2 = Utils.df_filter_by_expression_and_percentile(data, codes, item, 2, threshold)
            k_ven_percent_2, problems = Utils.vencode_percent_sampling(codes, celltype, filter_2, combinations_number_list,
        ...
        fig, path = Utils.errorbar_plot(k_ven_percent, folder, file_name, label=celltype, title=title, multiple=True)
        fig.savefig(path)
    else:
        ...
        if include_problems:
            ...
            Utils.write_dict_to_csv(new_file_name + ".csv", problems, folder)
    if not multi_plot:
        plt.close(fig)
    if init_data is None:
        print("Process Quick finished in %s seconds" % (time.time() - start_time))
...
def figure_2(file, file_type, cell_list, combinations_number, vens_to_take, reps, threshold=90,
    ...
    for celltype in cell_list:
        ...
        if isinstance(celltype_exclude, dict):
            celltype_to_exclude = celltype_exclude.get(celltype)
            codes_exclude = Utils.fantom_code_selector(file_type, data, celltype_to_exclude)
            try:
                for code in codes:
                    codes_exclude = [x for x in codes_exclude if x != code]
            except ValueError:
        ...
        filter_2 = Utils.df_filter_by_expression_and_percentile(data, codes, expression, 2, threshold)
        if get_vencodes:
            false_negatives, vencodes = Utils.vencode_percent_sampling_monte_carlo(codes, filter_2, combinations_number,
                                                                                   vens_to_take, reps, vencodes=get_vencodes)
        else:
            false_negatives = Utils.vencode_percent_sampling_monte_carlo(codes, filter_2, combinations_number, vens_to_take,
    ...
    Utils.write_dict_to_csv(file_name + ".csv", final, folder)
    fig_2, path_2 = Utils.box_plotting_from_dict(final, file_name, folder, title, keys_horizontal=True)
    fig_2.savefig(path_2, bbox_inches='tight')
    plt.close(fig_2)
...
def figure_3(file, file_type, celltype, vens_to_take, combinations_number=4, sample_types="primary cells", expression=1,
    ...
    for i in range(1, len(codes)):
        ...
        Utils.write_dict_2_to_csv(file_name + ".csv", partial_dict, folder)
    ...
    Utils.write_dict_2_to_csv(file_name + ".csv", final_dict, folder)
    print("Process Quick finished in %s seconds" % (time.time() - start_time))
    return
...
def figure_3_b(file, file_type, celltype, vens_to_take, combinations_number=4, sample_types="primary cells",
    ...
    for code in codes:
        ...
        Utils.write_one_value_dict_to_csv(file_name + ".csv", partial_perc_not_ven, folder)
